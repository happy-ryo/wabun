...
export const MARKERS = {
  ...
  "文頭「これは/この〜は」": {
    // 「この<名詞句>は」だけを標的にする。窓から読点・句点・引用符・空白を
    // 除外し、「この修正で「〜は」のように助詞や引用を飛び越えて後方の
    // 「は」を拾う誤検出を防ぐ(「この文書には」は名詞句+に+は なので残る)
    head: /^(これ|それ|この[^、。「」\sはがを]{0,12}|本[^、。「」\sはがを]{0,10})[はが]/,
    base: 2.4, trans: 15.4, warn: 6,
